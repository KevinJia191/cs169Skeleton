...
function Recipe(username, recipe_name, date_created, rating){
    ...
    this.add = function(postRequest, callback) {
        var jsonObject = {};     
            this.connection.query("SELECT * FROM users U WHERE username=\'" + postRequest.user + "/'", function(err, result){
                if(result.rows.length>0){
                    this.connection.query("SELECT * FROM history H WHERE username=\'" + postRequest.user + "\'" + "AND recipe_name=\'" + postRequest.recipe_name + "\'" + "AND current_date=" + postRequest.current_date, function(err, result){
                        if(result.rows.length == 0){
                            //Did not fail already made today check
                            this.connection.query("INSERT INTO HISTORY VALUES(" + postRequest.user + "," + postRequest.recipe_name + "," + postRequest.current_date + "," + postRequest.rating + ")", function(err, result){
                                done();
                                if(err){
                                    console.error(err);
